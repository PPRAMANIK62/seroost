...
fn serve_static_file(request: Request, file_path: &str, content_type: &str) -> Result<(), ()> {
    ...
    request
        .respond(response)
        .map_err(|err| eprintln!("ERROR: could not serve statis file {file_path}: {err}"))
}
...
fn serve_api_search(tf_index: &TermFreqIndex, mut request: Request) -> Result<(), ()> {
    ...
    let json = serde_json::to_string(&result.iter().take(20).collect::<Vec<_>>())
        .map_err(|err| eprintln!("ERROR: could not convert search results to JSON: {err}"))?;
...
pub fn start(address: &str, tf_index: &TermFreqIndex) -> Result<(), ()> {
    let server = Server::http(&address)
        .map_err(|err| eprintln!("ERROR: could not start HTTP server at {address}: {err}"))?;
